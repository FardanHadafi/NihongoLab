import { db, levels, userProgress, users, questions, userStats } from '@nihongolab/db';
...
export class DashboardService {
    async getDashboard(userId) {
        ...
        return {
            user: {
                ...
                currentLevel: user.levelName && user.requiredExp !== null
                    ? {
                        name: user.levelName,
                        requiredExp: user.requiredExp
                    }
                    : null,
                nextLevel: nextLevel && nextLevel.requiredExp !== null
                    ? {
                        name: nextLevel.name,
            ...
            stats: {
                ...
                accuracyRate: statsData && statsData.totalAnswered > 0
    ...
    async getUserStats(userId) {
        ...
        const [calculated] = await db
            .select({
            totalAnswered: sql `count(*)::int`,
            correctAnswers: sql `sum(case when ${userProgress.isCorrect} then 1 else 0 end)::int`
        })
            .from(userProgress)
    ...
    async getLevelProgress(userId) {
        ...
        const levelProgressData = await Promise.all(allLevels.map(async (level) => {
            ...
            const [answeredCorrectResult] = await db
                ...
                .where(and(eq(userProgress.userId, userId), eq(questions.levelId, level.id), eq(userProgress.isCorrect, true)));
    ...
    async answerReviewQuestion(userId, questionId, userAnswer) {
        const [row] = await db
            .select({
    ...
    async getRecentActivity(userId) {
        ...
        const activityData = await db
            .select({
            date: sql `DATE(${userProgress.answeredAt})`,
            count: sql `count(*)::int`
        })
            .from(userProgress)
    ...
    async getQuestionsMastered(userId) {
    ...
    async getQuestionsNeedingReview(userId) {
        // Questions with multiple attempts OR questions answered incorrectly
        const [result] = await db
            .select({ count: sql `count(*)::int` })
            .from(userProgress)
            .where(and(eq(userProgress.userId, userId), sql `${userProgress.attempts} > 1 OR ${userProgress.isCorrect} = false`));
        return result?.count || 0;
    }
    async getReviewQuestions(userId) {
        return db
            .select({
            progressId: userProgress.id,
            ...
            .from(userProgress)
            .innerJoin(questions, eq(userProgress.questionId, questions.id))
            .where(and(eq(userProgress.userId, userId), or(
        // Legacy review logic
        sql `${userProgress.attempts} > 1 OR ${userProgress.isCorrect} = false`, 
        // SRS-ready logic
        and(isNull(userProgress.nextReviewAt), eq(userProgress.isCorrect, false)), lte(userProgress.nextReviewAt, new Date()))))
            .orderBy(desc(userProgress.nextReviewAt), desc(userProgress.lastAttemptedAt))
            .limit(20);
    }
    async getKanjiQuestionsByLevel(level) {
        return await db
            .select({
            id: questions.id,
            questionText: questions.questionText,
